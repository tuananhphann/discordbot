...
class Table:
    ...
    def checkTable(self, table_name: str) -> bool:
        ...
        table = self.cur.execute(f"SELECT name FROM sqlite_master WHERE type='table' AND name='{table_name}'").fetchall()
        if len(table) == 1: return True
        return False
    ...
    def query(self, query: str, commit = False):
        ...
        if (commit == True): self.con.commit()
        else:
