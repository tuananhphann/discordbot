...
class Audio:
    ...
    async def process_track(self, ctx: commands.Context, track: str, priority: bool = False):
        ...
        if self.current_song == None:
            ...
            if priority == True:
                ...
                _log.info(f"Added to playlist '{song.TITLE}' from query '{track}' at 'Next' position.")
                ...
                _log.info(f"Added to playlist '{song.TITLE}' from query '{track}' at position '{self.playlist.index(song)+1}'.")
    ...
    async def timeout_handle(self, ctx: commands.Context):
        if ctx.voice_client.is_playing():
            ...
            _log.info(f"Disconnected from voice channel due to timeout. ID: {ctx.author.voice.channel.id}")
